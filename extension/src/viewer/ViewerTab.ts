...
export class ViewerTab extends Disposable
{
    // private resource: vscode.Uri;
    public activeFile : CortexPreview; 
    private state: PreviewState = 'Visible';
    public readonly ID;
    constructor(
        private readonly extensionRoot: vscode.Uri,
        initialFile: CortexPreview,
    ...
    private setupWebview() 
    {
        this.panel.webview.options = {
            enableScripts: true,
            localResourceRoots: [
                vscode.Uri.file(path.join(this.extensionRoot.fsPath, 'lib')),
                vscode.Uri.file(path.dirname(this.activeFile.Source.fsPath)) //////nned to set this to the global ID!
            ]
        };


        //register disposable elements
        this._register(this.panel.webview.onDidReceiveMessage(      async (msg) =>  {this.messageHandler(msg);      }));
        this._register(this.panel.onDidChangeViewState(             () =>           {this.update();                 }));

		this._register(this.panel.onDidDispose(                     () =>           {this.state = 'Disposed';       }));

		// const watcher = this._register(vscode.workspace.createFileSystemWatcher(this.activeFile.Source.fsPath));
		// this._register(watcher.onDidChange(                         (e) =>          {this.matchAction(e,this.reload);}));
		// this._register(watcher.onDidDelete(                         (e) =>          {this.matchAction(e,this.panel.dispose);}));
		
    }

    public GetName() : string
    {
        return this.activeFile.Source.toString();
    }
    ...
    private matchAction(input: vscode.Uri, functionPtr: any): void
    {
        if (input.toString() === this.activeFile.Source.toString())
        {
		    functionPtr();
		}
    }
    ...
    public async show(preview: CortexPreview)
    {
    ...
    private getHtml(): string {
		...
		const docPath = webview.asWebviewUri(this.activeFile.Source);
...
<meta http-equiv="Content-Security-Policy" content="default-src 'none'; connect-src ${cspSource} blob; script-src 'unsafe-inline' ${cspSource}; style-src 'unsafe-inline' ${cspSource}; img-src blob: data: ${cspSource};">
