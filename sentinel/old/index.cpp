...
{
    Structure = std::make_shared<Directory>(Cortex.Settings.Files.TargetDirectory);
...
void FileIndex::Compile(bool forceAll)
{
    ...
    fs::path macros = (fs::path)Cortex.Settings.Files.TargetDirectory / macroFile;
...
{
    auto truePath = Cortex.Settings.Files.TargetDirectory / path;
